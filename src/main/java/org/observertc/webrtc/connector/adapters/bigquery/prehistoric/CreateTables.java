...
package org.observertc.webrtc.connector.adapters.bigquery.prehistoric;

import com.google.cloud.bigquery.*;
import org.observertc.webrtc.connector.sinks.bigquery.models.*;
...
public class CreateTables extends Job {
		...
		return new AbstractTask(CREATE_ICE_REMOTE_CANDIDATE_TABLE_TASK_NAME) {
			...
			protected void execute() {
				...
				Schema schema = Schema.of(
						...
						,
						Field.newBuilder(ICERemoteCandidateEntry.BROWSERID_FIELD_NAME, LegacySQLTypeName.STRING).setMode(Field.Mode.NULLABLE).build()
						,
		...
		return new AbstractTask(CREATE_MEDIA_SOURCES_TABLE_TASK_NAME) {
			...
			protected void execute() {
				...
				Schema schema = Schema.of(
						...
						,
						Field.newBuilder(MediaSourceEntry.BROWSERID_FIELD_NAME, LegacySQLTypeName.STRING).setMode(Field.Mode.NULLABLE).build()
						,
	...
	private Task makeObserverEventTableTask() {
		...
		return new AbstractTask(CREATE_OBSERVER_EVENT_TABLE_TASK_NAME) {
			...
			@Override
			protected void execute() {
				String observerEventTables = tableNames.get(EntryType.ObserverEvent);
				if (Objects.isNull(observerEventTables)) {
					logger.warn("Table name for entry type {} has not been declared for {}", EntryType.ObserverEvent, CreateTables.class.getSimpleName());
					return;
				}
				TableId tableId = TableId.of(projectId, datasetId, observerEventTables);
				Schema schema = Schema.of(
